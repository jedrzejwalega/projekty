...
def main(path_to_blast_table, email, e_value_maximum_threshold=None, database="nuccore", desired_file_name="overlapping_genes"):
    ...
    write_hit_params_to_files(filtered_blast_table)
    sequences = get_sequences(email, database)
...
def write_hit_params_to_files(filtered_blast_table):
    write_only_acc_keys(filtered_blast_table)
    write_all_three_params(filtered_blast_table)


# create a new file "blast_results_only_acc_keys" with accession keys to blast results delimited by \n
def write_only_acc_keys(filtered_blast_table):
    only_acc_keys = filtered_blast_table["subject acc.ver"]
    with open("blast_results_only_acc_keys", "w") as handle:
...
def write_all_three_params(filtered_blast_table):
    all_three_params = filtered_blast_table[["subject acc.ver", "s. start", "s. end"]]

    with open("blast_results_all_three_params", "w") as handle:
...
def get_sequences(email, database):
    sequences = download_from_NCBI(email, database=database)
    write_sequences_to_file(sequences)
    return sequences


# Download sequences from NCBI
def download_from_NCBI(email, database):
    Entrez.email = email
    accession_keys = get_accession_keys()
    handle = Entrez.efetch(db=database, id=accession_keys, rettype="gb")
...
def get_accession_keys():
    with open("blast_results_only_acc_keys", "r") as handle:
...
def read_hsp_file():
    hsp_parameters = []
    with open("blast_results_all_three_params", "r") as handle:
...
# Function call
# print(main("/home/jedrzej/Pobrane/6D9N0W8Z014-Alignment-HitTable.csv", "jedrzejwalega@gmail.com", e_value_maximum_threshold=1, desired_file_name="blastx_testing", database="protein"))
# print(main("/home/jedrzej/Pobrane/2ASZVDWK016-Alignment-HitTable.csv", "jedrzejwalega@gmail.com", e_value_maximum_threshold=10**(-50), desired_file_name="final_testing"))
print(main("/home/jedrzej/Desktop/7USZAH76014-Alignment-HitTable.csv", "jedrzejwalega@gmail.com", e_value_maximum_threshold=5.72*10**(-180), desired_file_name="skywalker"))